...
def get_user():
    ...
    # proc = subprocess.Popen("php getenv.php", shell=True, stdout=subprocess.PIPE)
    # script_response = str(proc.stdout.read())
    # print(script_response)
    # envs = script_response.split("{eraldaja}")
    # ident = envs[0]
    # verify = envs[1]

    stuff.append("ident: " + str(ident) + "; verify: " + str(verify))

    # check and parse the values
    if not ident or verify != "SUCCESS":
        return None

    ident = str2utf8(ident) # old cards use UCS-2, new cards use UTF-8

    stuff.append("ident: " + str(ident))

    if not ident.startswith("/C=EE/O=ESTEID"):
        return None
    ...
    stuff.append("ps: " + str(ps) + "; pg: " + str(pg) + "; pc: " + str(pc))

    if ps <= 0 or pg <= 0 or pc <= 0:
        return None

    res = [ident[ps+4:pg], ident[pg+4:pc], ident[pc+14:]]

    stuff.append("res: " + str(res))
